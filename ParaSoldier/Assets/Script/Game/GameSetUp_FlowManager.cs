...
public class GameSetUp_FlowManager : MonoBehaviour
{
    ...
    async UniTask Init()
    {
        ...
        GameManager.Instance.PlayerTransform = nowPlayerCharacter.transform;
    ...
    async UniTask Enemy_StageSetUp()
    {
        ...
        GameManager.Instance.EnemyTransform = nowEnemy.transform;
        // 反転させる
    ...
    private void SyncCharacterStatus(CharacterStatus status, PType type)
    {
        ...
        if (type == PType.P1)
        {
            ...
            status.hp
                ...
                .Subscribe(x => hpManager.p1Hp.Value = x)
                .AddTo(this); // ← status ではなく FlowManager に紐付ける

            status.gage.Value = hpManager.p1Gage.Value;

            status.gage
                .DistinctUntilChanged()
                .Subscribe(x => hpManager.p1Gage.Value = x)
                .AddTo(this);
        }
        else if (type == PType.P2)
        {
            ...
            status.hp
                .DistinctUntilChanged() // 同じ値なら無視
                .Subscribe(x => hpManager.p2Hp.Value = x)
                .AddTo(this); // ← status ではなく FlowManager に紐付ける
            ...
            status.gage
                .DistinctUntilChanged()
                .Subscribe(x => hpManager.p2Gage.Value = x)
                .AddTo(this);
        }
    }
